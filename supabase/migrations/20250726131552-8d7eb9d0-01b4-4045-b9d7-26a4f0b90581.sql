-- Fix remaining security issues (corrected syntax)
...
-- 5. Add constraints for data integrity (without IF NOT EXISTS)
...
ALTER TABLE public.cargas DROP CONSTRAINT IF EXISTS valid_estado;
ALTER TABLE public.cargas 
ADD CONSTRAINT valid_estado 
CHECK (estado IN ('disponible', 'asignada', 'completada', 'cancelada'));

ALTER TABLE public.camiones_disponibles DROP CONSTRAINT IF EXISTS valid_truck_estado;
